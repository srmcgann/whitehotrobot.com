  ...
  data(){
    return{
      ...
      state: {
        ...
        networks: [],
        defaultNetwork: {
          name: 'irc.whitehotrobot.com',
          port: 6667,
          connected: false,
          nick: 'whr_user_' + (10+Math.random()*90|0),
          channels: null,
        },
        networkTemplate: {
          name: '',
        ...
        channels:[
          {name: 'server', connected: false, highlighted: false, active: true, buffer: [], newText: false, scrollStick: true, history: [], users: [], topic: '', showServerMessages: true, networkID: null}
        ],
        defaultChannels:[
          {name: '#help', connected: false, highlighted: false, active: false, buffer: [], newText: false, scrollStick: true, history: [], users: [], topic: '', showServerMessages: false, networkID: null},
          {name: '#whitehotrobot', connected: false, highlighted: false, active: false, buffer: [], newText: false, scrollStick: true, history: [], users: [], topic: '', showServerMessages: false, networkID: null},
          {name: '#math', connected: false, highlighted: false, active: false, buffer: [], newText: false, scrollStick: true, history: [], users: [], topic: '', showServerMessages: false, networkID: null},
          {name: '#javascript', connected: false, highlighted: false, active: false, buffer: [], newText: false, scrollStick: true, history: [], users: [], topic: '', showServerMessages: false, networkID: null},
          {name: '#music', connected: false, highlighted: false, active: false, buffer: [], newText: false, scrollStick: true, history: [], users: [], topic: '', showServerMessages: false, networkID: null},
          {name: '#literature', connected: false, highlighted: false, active: false, buffer: [], newText: false, scrollStick: true, history: [], users: [], topic: '', showServerMessages: false, networkID: null},
          {name: '#gamedev', connected: false, highlighted: false, active: false, buffer: [], newText: false, scrollStick: true, history: [], users: [], topic: '', showServerMessages: false, networkID: null},
          {name: '#audiocloud', connected: false, highlighted: false, active: false, buffer: [], newText: false, scrollStick: true, history: [], users: [], topic: '', showServerMessages: false, networkID: null},
          {name: '#words', connected: false, highlighted: false, active: false, buffer: [], newText: false, scrollStick: true, history: [], users: [], topic: '', showServerMessages: false, networkID: null},
          {name: '#code', connected: false, highlighted: false, active: false, buffer: [], newText: false, scrollStick: true, history: [], users: [], topic: '', showServerMessages: false, networkID: null}
        ],
        supressFooter: false,
        //defaultIRCHost: 'irc.whitehotrobot.com',
        //defaultIRCPort: 6667,
        baseURL: 'https://chat.whitehotrobot.com',
  ...
  methods:{
    ...
    getMode(){
      ...
      vars.map((v,i)=>{
        ...
        if(!tnet) tnet = this.state.defaultNetwork.name
        if(!tport) tport = this.state.defaultNetwork.port
        if(!tnick) tnick = this.state.defaultNetwork.nick
        if(!tchannels.length){
          tchannels = JSON.parse(JSON.stringify(this.state.defaultChannels)).map(v=>v.networkID = this.state.networks.length)
        } else {
          tchannels = tchannels.map(v=>{
            let newChan = JSON.parse(JSON.stringify(this.state.channelTemplate))
            newChan.name = v
            newChan.networkID = this.state.networks.length
            return newChan
          })
        }
        newNet.name = tnet
        newNet.port = tport
        newNet.nick = tnick
        newNet.channels = tchannels
        this.state.networks.push(newNet)
      })
      this.state.networks = this.state.networks.map((v, i)=>{
        let serverChan = JSON.parse(JSON.stringify(this.state.channelTemplate))
        serverChan.name = 'server'
        serverChan.networkID = i
        v.channels = [serverChan, ...v.channels]
        return v
      })
      if(vars.length>0){
        switch(vars[0]){
        ...
        .then(data => {
          ...
          if(+data[0]){
            if(!this.state.loggedin){
              ...
              this.state.networks.map(v=>this.state.serverConnect(v))
            }
    ...
    pushToBuffer(channel, msg, mode){
      if(this.state.loggedin && msg && channel.connected){
        if(msg.split(':').length > 2 && msg.split(':')[1].split(' ')[0].toUpperCase() == this.state.networks[channel.networkID].name.toUpperCase()){
          let t = 0
          let idx = 0
          msg.split('').map((v, i)=>{
            if(v==':'){
    ...
    getNick(network){
      if(network.nick) return network.nick
      let nick = this.state.defaultNetwork.nick
    ...
    doActions(msg, network){
      ...
      switch(this.getMsgType(msg)){
        ...
        case 'ping':
          ...
          if(msg.toUpperCase().indexOf(network.name.toUpperCase()) === -1 && !network.connected){
            setTimeout(()=>{
        ...
        case 'privmsg':
          ...
          if(from.toUpperCase() != network.name.toUpperCase() && to.toUpperCase() === this.state.getNick().toUpperCase()){
        ...
        case 'join':
          ...
          console.log('join detected: ' + joinChan + ' ' + joinUser)
          let newUser = JSON.parse(JSON.stringify(this.state.userTemplate))
        ...
        switch(response.type){
          case 'server_message':
            ...
            res.split("\n").map(msg=>{
              ...
              if(msg.length > 1){
                console.log('server['+network.name+']: ', msg)
                this.doActions(msg, network)
              }
            })
          break;
        }
    ...
    serverConnect(network){
      let host = network.name
      ...
      if(host && port && nick){
        ...
        xhr.onprogress = e => {
          this.handleServer(e.currentTarget.responseText, network)
        }
