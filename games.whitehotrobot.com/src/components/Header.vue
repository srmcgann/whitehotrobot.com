<template>
  <div class="header" id="header">
    ...
    <div v-else style="display: inline-block; position: absolute;">
      <button :class="{'bumpDown': state.mode == 'post'}" @click="createPost()" class="createPostButton">create post</button>
    </div>
    <div class="curPageContainer" v-if="(state.totalPages > 0 || state.totalUserPages > 0) && state.mode != 'post' || (state.search.string && state.totalPages>1)" :class="{'bumpLeft': !state.loggedin}">
      <button
        class="navButton"
    ...
    <div class="workingSpace">
      ...
      <a :href="origin" class="appName">
        <div class="appNameText">
          ...
          <div style="transform: scale(1.7, 1);margin-top: -5px;margin-right: 25px">games</div>
        </div>
        <div class="clear"></div>
      </a>
    </div>
...
export default {
  ...
  methods:{
    ...
    createPost(){
      ...
      fetch(this.state.baseURL + '/createPost.php',{
        method: 'POST',
        headers: {
        ...
        console.log(data)
        if(data[0]) window.location.href = this.state.baseURL + '/post/' + this.state.decToAlpha(data[1])
      })
    }
  },
  mounted(){
  ...
  computed:{
    totalPages(){
      switch(this.state.mode){
        ...
        case 'post': return +this.state.totalPages; break
      }
    },
    curPage(){
      switch(this.state.mode){
        ...
        case 'post': return +this.state.curPage; break
      }
    },
    pagenumber(){
      let num
...
.header{
  ...
  height: 60px;
  width: 100%;
  background: linear-gradient(90deg, #0008, #043c, #0008);
