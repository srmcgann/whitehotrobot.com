<template>
  <div class="controls" v-if="!state.showUploadModal" :class="{'shortControlsContainer':!state.showControls}">
    <div class="controlsWorkingSpace">
      <div @click="state.toggleShowControls" v-if="state.mode !== 'post'" title="show additional controls [ESC]" :class="{'showImg':!state.showControls, 'hideImg': state.showControls, 'bump': state.mode=='post'}" style="margin-top:-70px;"></div>
      <transition name="fade">
        <div v-if="state.showControls">
          <div class="navContainer">
            <div v-if="state.mode !== 'post'" class="advancedControls">
              <div style="position: absolute; z-index:-1!important;background: #254;width: 505px;height: 70px;opacity: .5;margin-top: -10px;margin-left: 60px;border-radius: 5px;"></div>
              ...
              <select v-if="state.loggedin && state.mode != 'post'" style="font-size: 16px;margin-left: 20px;vertical-align: top;margin-top: 4px;border: 1px solid #8ff4;display: inline-block;position: relative;" v-model="state.maxResultsPerPage" @input="updateUserPrefs('gamesPostsPerPage')">
  ...
  methods:{
    ...
    updateUserPrefs(pref){
      this.$nextTick(()=>{
        let newval
        switch(pref){
          case 'gamesPostsPerPage': newval = this.state.maxResultsPerPage; break
        }
        let sendData = {
       ...
       fetch(this.state.baseURL + '/updatePrefs.php',{
        ...
        .then(res => res.json())
        .then(data => {
          if(pref == 'gamesPostsPerPage') window.location.reload()
        })
      })
    },
    doReg(){
...
.controlsWorkingSpace .controlsbutton{
  background: linear-gradient(90deg, #166d, #145d);
  color: #def;
  margin-top: -20px;
  margin-right:60px;
  margin-left:60px;
