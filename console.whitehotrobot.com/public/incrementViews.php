<?
  ...
  $sql = "DELETE FROM consoleviews WHERE time < " . $t_minus;
  ...
  if($_SERVER['REMOTE_ADDR']){
    ...
    $sql='SELECT time FROM consoleviews WHERE decIP = ' . $decIP . ' AND demoID = ' . $demoID . ' ORDER BY time DESC';
    $sql1=$sql;
    ...
    if($go){
      $row = mysqli_fetch_assoc($res);
      $sql = 'UPDATE consoleitems SET views = views + 1 WHERE id = ' . $demoID;
      mysqli_query($link, $sql);
      $time = time();
      $sql = 'INSERT INTO consoleviews (decIP, demoID, time) VALUES('.$decIP.','.$demoID.','.$time.');';
      mysqli_query($link, $sql);
    }
  }
  echo json_encode([$go]);
