<?
  ...
  if($name){
    ...
    if($includePrivate){
      ...
      $sql="SELECT id FROM consoleitems WHERE userID = " . $row['id'];
      $res = mysqli_query($link, $sql);
      $totalRecords = mysqli_num_rows($res);
      $totalPages = (($totalRecords-1) / $maxResultsPerPage | 0) + 1;
  
      $sql1=$sql = "SELECT * FROM consoleitems WHERE userID = " . $row['id'] . ' ORDER BY created DESC LIMIT ' . $start . ', ' . $maxResultsPerPage;
    } else {

      $sql="SELECT id FROM consoleitems WHERE private = 0 AND userID = " . $row['id'];
      $res = mysqli_query($link, $sql);
      $totalRecords = mysqli_num_rows($res);
      $morePages = false;
      $totalPages = (($totalRecords-1) / $maxResultsPerPage | 0) + 1;
      ...
      $sql = "SELECT * FROM consoleitems WHERE userID = " . $row['id'] . ' AND private = 0 ORDER BY id DESC LIMIT ' . $start . ', ' . $maxResultsPerPage;
    }
    $res = mysqli_query($link, $sql);
    $demos = [];
    ...
    forEach($demos as &$demo){
      $demoID = $demo['id'];
      $sql = 'SELECT * FROM consoleComments WHERE demoID = ' . $demoID;
