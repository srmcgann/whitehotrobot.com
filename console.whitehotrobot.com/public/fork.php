<?
  ...
  function previousForkID($id){
    global $link;
    $_sql = "SELECT forkedFrom FROM consoleitems WHERE id = " . $id;
  ...
  function demoExists($id){
    global $link;
    $_sql = 'SELECT id FROM consoleitems WHERE id = ' . $id;
    $_res = mysqli_query($link, $_sql);
  ...
  foreach($a as $id){
    if(demoExists($id)){
        ...
        $sql = 'INSERT INTO consoleitems (videoLink, demoJS, demoCSS, demoHTML, title, userID, author, forkHistory) VALUES("'.$videoLink.'","'.$demoJS.'","'.$demoCSS.'","'.$demoHTML.'","'.$title.'",'.$userID.',"'.$author.'","'.$forkHistory.'")';
