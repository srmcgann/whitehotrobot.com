<?
  ...
  if(sizeof($tokens)){
    ...
    if($loggedinUserName && $confirmed){
      $sql = 'SELECT * FROM consoleitems WHERE (private = 0 || userID = '.$loggedinUserData['id'].') AND ((title LIKE "%' . $tokens[0] . '%"';
    }else{
      $sql = 'SELECT * FROM consoleitems WHERE private = 0 AND ((title LIKE "%' . $tokens[0] . '%"';
    }
    if(sizeof($tokens>1)){
      array_shift($tokens);
