<?
  ...
  if(mysqli_num_rows($res)){
    ...
    if($row['enabled']){
      if($row['admin']){
        $sql = 'UPDATE consoleitems SET demoHTML = "'.$demoHTML.'", demoCSS = "'.$demoCSS.'", demoJS = "'.$demoJS.'" WHERE id = '.$demoID;
      } else {
        $sql = 'UPDATE consoleitems SET demoHTML = "'.$demoHTML.'", demoCSS = "'.$demoCSS.'", demoJS = "'.$demoJS.'" WHERE id = '.$demoID . ' AND userID = ' . $row['id'];
      }
