<?
  ...
  $demoID = mysqli_real_escape_string($link, $data->{'itemID'});
  ...
  if(mysqli_num_rows($res)){
    ...
    if($row['enabled']){
      $sql = 'INSERT INTO demoComments (text, demoID, userID) VALUES("'.$comment.'", '.$demoID.', '.$row['id'].')';
  ...
  if($success){
		...
		$res = mysqli_query($link, $sql);
		$row = mysqli_fetch_assoc($res);
		echo json_encode([$success, $insertID, $row['date']]);
	} else {
