  ...
  methods:{
    ...
    filterLine(lineObject){
      ...
      if(line.indexOf(String.fromCharCode(3))!==-1){
        ...
        lines.map((v,i)=>{
          ...
          newLine += '<span class="color' + num + '">' + v + '</span>'
    ...
    setChatWindowHeight(){
      ...
      if(navTabs){
        ...
        let textLinesTop
        if(document.querySelectorAll('.textLines').length){
          if(this.state.curChannelId && this.$refs.channelInfo) textLinesTop = this.$refs.channelInfo.getBoundingClientRect().top + this.$refs.channelInfo.clientHeight
          document.querySelectorAll('.textLines').forEach((el, idx)=>{
            if(idx){
              el.style.top = (textLinesTop - 5) + 'px'
              el.style.height = (inputBar.getBoundingClientRect().top - textLinesTop) + 'px'
            } else {
              el.style.height = (inputBar.getBoundingClientRect().top - el.getBoundingClientRect().top) + 'px'
            }
          })
        }
